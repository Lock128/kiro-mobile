...
/// A fake [WebViewPlatform] so that [WebViewCookieManager] can be instantiated
...
/// Minimal fake cookie manager that satisfies the platform interface.
...
void main() {
  ...
  /// For any AuthManager in an authenticated state, calling signOut() should
  /// clear all credentials from the CredentialStore and transition the
  ...
  Glados(any.validAuthCredentials, ExploreConfig(numRuns: 100)).test(
    ...
    (AuthCredentials credentials) async {
      ...
      expect(authManager.state, equals(AuthState.authenticated),
          reason: 'AuthManager should be authenticated after init with valid credentials');
      ...
      expect(store.clearCalled, isTrue,
          reason: 'clear() should be called during sign-out');
      expect(store.storedCredentials, isNull,
          reason: 'Stored credentials should be null after sign-out');
